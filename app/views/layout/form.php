...
<form role="form" method="post" autocomplete="off" parsley-validate novalidate>
    ...
    <div class="col-md-3">
        <div class="form-group" style="line-height:34px">
            ...
            <input type="text" class="form-control" name="data[name]" placeholder="" required parsley-error-message="Add name for layout" value="<?php echo htmlspecialchars($this->model->name); ?>">
        </div>
        <div class="form-group" style="line-height:34px">
            <input type="submit" class="btn btn-primary" value="<?php echo ($this->model->isNew() ? 'Create': 'Save'); ?>" /> or <a href="<?php echo $this->app->createUrl(array('layout', 'index')); ?>">Cancel</a>
        </div>
    </div>
</form>